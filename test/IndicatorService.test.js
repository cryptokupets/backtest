...
describe("IndicatorService", () => {
    it("execute", function (done) {
        const { candles, indicatorOutputs } = JSON.parse(
        ...
        IndicatorService.execute(options)
            .then((output) => {
                // console.log(output);
                ...
                for (let index = 0; index < output.length; index++) {
                    assert.equal(
                        indicatorOutputs[index].time,
                        output[index].time
                    );
                    assert.equal(
                        indicatorOutputs[index].values[0],
                        Math.round(output[index].values[0] * 100) / 100
                    );
                }
                done();
            })
